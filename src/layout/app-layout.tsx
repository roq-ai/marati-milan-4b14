...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiHeart } from 'react-icons/fi';
...
import { getMatrimonies } from 'apiSdk/matrimonies';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['registered-user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MatrimonyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/matrimonies` : null),
    () => getMatrimonies({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Matrimony',
      path: isTenantUser ? `/matrimonies/view/${data?.[0]?.id}` : '/matrimonies',
      entity: 'matrimony',
